...
def alert_duplicate_streak(name, value, streak):
    ...
    2026-09-02 멘트 수정 (재인님 요청): 재검증 예고 문구를 더 짧고 친근한 톤으로 변경."""
    send_alert(
        f"`{name}` 값이 {streak}번 연속 동일한 값으로 기록되고 있어요.\n"
        f"월물 만기 혹은 데이터 소스에 문제가 있을 수 있으니 확인해주세요!\n"
        f"⏳ 10분 뒤 한 번 더 체크할게요 :) (오류였다면 다시 알려드릴게요!)\n"
        f"➡️ 동일한 진폭 : {value}",
        title="🚨진폭 값 반복 감지🚨",
    )
...
def alert_streak_recheck_correction(name, date_str, time_str, old_val, new_val):
    ...
    2026-09-02 멘트 수정 (재인님 요청): 원인 설명은 줄이고 결과 위주로 더 간결하게 변경."""
    send_alert(
        f"`{name}` {date_str} {time_str} 재검증 완료!\n"
        f"일시적 오류로 값이 달라져서 다시 알려드려요. (스프레드시트 수정 완료)\n"
